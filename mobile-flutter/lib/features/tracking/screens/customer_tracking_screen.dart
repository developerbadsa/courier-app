...
class _CustomerTrackingScreenState extends State<CustomerTrackingScreen> {
  ...
  void initState() {
    super.initState();
    _trackingController = TextEditingController(text: widget.initialTrackingNumber ?? 'SHN-9482-US');
  ...
  Future<void> _onSearch(String query) async {
    ...
    try {
      ...
      if (mounted && response.data?['data'] != null) {
        setState(() { _isSearching = false; _parcelData = response.data['data']; });
        return;
      }
    } catch (_) {}
    ...
    setState(() {
      ...
      _parcelData = {
        ...
        'events': [
          {'title': 'Out for Delivery', 'time': '10:15 AM, Today', 'isDone': true, 'active': true},
          {'title': 'At Sorting Facility', 'time': '07:30 AM, Today', 'isDone': true, 'active': false},
          {'title': 'Picked Up from Warehouse', 'time': '05:45 PM, Yesterday', 'isDone': true, 'active': false},
          {'title': 'Manifest Created', 'time': '02:00 PM, Yesterday', 'isDone': true, 'active': false},
        ],
      };
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: const Text('Live Shipment Tracker', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: Colors.white)),
      ),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  AppButton(text: 'Track', isLoading: _isSearching, onPressed: () => _onSearch(_trackingController.text)),
                ],
              ),
            ),
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, idx) {
                  ...
                  return GestureDetector(
                    onTap: () { _trackingController.text = code; _onSearch(code); },
                    child: Container(
            ...
            if (_parcelData != null) ...[
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [
                    BoxShadow(color: AppColors.primary.withValues(alpha: 0.25), blurRadius: 14, offset: const Offset(0, 4)),
                  ],
                ),
                child: Column(
                  ...
                  children: [
                    Row(
                      ...
                      children: [
                        Text(_parcelData!['trackingNumber'] ?? '', style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900, fontFamily: 'monospace')),
                    ...
                    Row(
                      children: [
                        const Icon(LucideIcons.calendarClock, color: Colors.white70, size: 16),
                        const SizedBox(width: 8),
                        Text('ETA: ${_parcelData!['estimatedDelivery'] ?? 'Today'}', style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Text('Destination: ${addr?['line1'] ?? 'N/A'}, ${addr?['city'] ?? 'N/A'}',
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(LucideIcons.bike, color: AppColors.primary, size: 22),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(rider['name'] ?? 'Courier', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13.5)),
                            const SizedBox(height: 2),
                            Text('${rider['vehicle']} • ETA ~18 mins', style: const TextStyle(color: AppColors.textMuted, fontSize: 11.5)),
                          ],
                        ),
                      ),
                      IconButton.filled(
              ...
              AppCard(
                child: Column(
                  ...
                  children: [
                    const Row(
                      children: [
                        Icon(LucideIcons.milestone, color: AppColors.primary, size: 18),
                        SizedBox(width: 8),
                        Text('Delivery Timeline', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                      ],
                    ),
                    const SizedBox(height: 16),
                    ListView.builder(
                              ...
                              children: [
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                    ...
                                    color: isActive ? AppColors.primary : (ev['isDone'] ? AppColors.success : AppColors.textLight),
                                  ),
                                  child: Icon(ev['isDone'] ? Icons.check : Icons.circle, size: 11, color: Colors.white),
                                ),
                                if (!isLast)
                                  Container(width: 2, height: 34, color: ev['isDone'] ? AppColors.success.withValues(alpha: 0.3) : AppColors.border),
                              ],
                            ),
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  const SizedBox(height: 2),
                                  Text(ev['time'] ?? '', style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
                                  const SizedBox(height: 16),
                                ],
                              ),
                            ),
              ...
              AppCard(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Shipment Details', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                    ...
                    _buildMetaRow(
                      ...
                      (_parcelData!['codAmount'] as double? ?? 0) > 0
                          ? 'COD: \$${(_parcelData!['codAmount'] as double).toStringAsFixed(2)}'
                          : 'Prepaid',
                      isHighlight: true,
                    ),
                  ],
      ...
      child: Row(
        ...
        children: [
          ...
          Text(value, style: TextStyle(
            ...
            fontWeight: isHighlight ? FontWeight.bold : FontWeight.w600, fontSize: 12,
          )),
        ],
      ),
    );
