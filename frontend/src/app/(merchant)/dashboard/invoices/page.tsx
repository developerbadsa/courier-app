...
import {
  ...
  Package, Printer, Loader2, CheckCircle2, Clock, Plus, Filter,
} from 'lucide-react';
import { DashboardLayout } from '@/components/layout';
import { StatCard, Button, Badge, Card, Modal, Input } from '@/components/ui';
...
export default function InvoicesPage() {
  ...
  return (
    ...
    >
      ...
      {/* ── Invoice Cards List with my-8 and space-y-6 ── */}
      <div className="my-8 space-y-6">
        {filtered.length === 0 ? (
          ...
          filtered.map((invoice) => (
            <Card key={invoice.id} className="p-6 hover:shadow-md transition-all border-slate-200 bg-white">
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div className="flex-1 space-y-2">
                  <div className="flex items-center gap-3">
                    <span className="text-base font-bold text-slate-900 tracking-tight">{invoice.number}</span>
                    <span className={`text-[10px] px-3 py-0.5 rounded-full font-bold border ${statusStyle(invoice.status)}`}>
                      {invoice.status}
                    </span>
                  </div>
                  <div className="text-xs text-slate-500 flex flex-wrap items-center gap-4 pt-1">
                    <span className="flex items-center gap-1.5 font-medium text-slate-600">
                      <Calendar className="w-3.5 h-3.5 text-slate-400" /> {invoice.period}
                    </span>
                    <span className="flex items-center gap-1.5 font-medium text-slate-600">
                      <Package className="w-3.5 h-3.5 text-slate-400" /> {invoice.shipmentCount} shipments included
                    </span>
                  </div>
                  <div className="flex items-center gap-6 pt-1 text-xs text-slate-500">
                    <span>Issued Date: <strong className="text-slate-700">{new Date(invoice.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</strong></span>
                    <span>Due Date: <strong className="text-slate-700">{new Date(invoice.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</strong></span>
                  </div>
                </div>

                <div className="sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100">
                  <div className="text-2xl font-black text-slate-900 tracking-tight">${invoice.total.toFixed(2)}</div>
                  <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Total USD</div>
                </div>
              </div>

              {/* Items Breakdown Accordion Box with generous padding */}
              <div className="mt-6 pt-4 border-t border-slate-100 bg-slate-50/80 p-4 rounded-lg">
